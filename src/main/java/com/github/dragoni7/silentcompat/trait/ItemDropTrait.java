...
public class ItemDropTrait extends SimpleTrait {
	...
	public static final Serializer<ItemDropTrait> SERIALIZER = new Serializer<ItemDropTrait>(SilentCompat.getId("item_drop"),
			ItemDropTrait::new,
			ItemDropTrait::readJson,
			ItemDropTrait::read,
			ItemDropTrait::write);
    ...
    public float onDurabilityDamage(TraitActionContext context, int damageTaken) {
		...
		if (level.getRandom().nextInt(chance) % chance == 0) {
			if (!level.isClientSide) {
				...
				level.addFreshEntity(new ItemEntity(level, pos.x(), pos.y(), pos.z(), new ItemStack(dropItem), d0, d1, d2));
			}
		}
    ...
    public Collection<String> getExtraWikiLines() {
        ...
        if (dropItem != null) {
        	ret.add("Chance to drop" + NameUtils.fromItem(dropItem) + " on durability loss with 1 out of" + chance + "% chance");
        }
